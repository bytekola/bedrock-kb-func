...
export class BotStore extends Construct {
  ...
  constructor(scope: Construct, id: string, props: BotStoreProps) {
    ...
    const vpc: IVpc = Vpc.fromLookup(this, 'ImportVpc', {
      vpcId: props.vpcId,
    });

    const openSearchSecurityGroup = new SecurityGroup(this, 'OpenSearchSecurityGroup', {
      vpc: vpc,
      securityGroupName: "chatbot-opensearch-sg",
      description: 'Allow HTTP and SSH access',
      allowAllOutbound: true
    });

    openSearchSecurityGroup.addIngressRule(Peer.ipv4("10.0.0.0/8"), Port.tcp(443))

    const collectionName = generatePhysicalName(this, `${props.envPrefix}Collection`, {
      maxLength: 32,
      lower: true,
    });

    const standbyReplicas = props.enableBotStoreReplicas === true ? "ENABLED" : "DISABLED";
