...
export class Auth extends Construct {
  ...
  constructor(scope: Construct, id: string, props: AuthProps) {
    ...
    if (props.allowedSignUpEmailDomains.length >= 1) {
      ...
      checkEmailDomainFunction.role?.addManagedPolicy(
    ...
    const publishAllowedGroup = new CfnUserPoolGroup(this, "PublishAllowedGroup", {
    ...
    if (props.autoJoinUserGroups.length >= 1) {
      ...
      addUserToGroupsFunction.role?.addManagedPolicy(
        iam.ManagedPolicy.fromAwsManagedPolicyName(
            "service-role/AWSLambdaVPCAccessExecutionRole"
        )
      )
      ...
      cognitoTriggerRegistrationFunction.role?.addManagedPolicy(
        iam.ManagedPolicy.fromAwsManagedPolicyName(
          "service-role/AWSLambdaVPCAccessExecutionRole"
        )
      )
