...
const BaseParametersSchema = z.object({
  ...
  bedrockRegion: z.string().default("eu-west-2"),
...
const BedrockChatParametersSchema = BaseParametersSchema.extend({
  ...
  enableLambdaSnapStart: z.boolean().default(false),
  ...
  // BotStore
  enableBotStore: z.boolean().default(false),
  ...
  lb_subnets: z.any().default([]),
  
  
  //? Vpc And Subnets
  vpcId: z.string().default(""),
  subnets: z.any().default([]),
  executeApiEndpointId: z.string().default(""),
  s3VpcEndpoint: z.string().default(""),
  s3EndpointIps: z.any().default([]),
  albIngressCidr: z.string().default("10.0.0.0/8"),
  openSearchVpcEndpoint: z.string().default("")
});
...
export function resolveBedrockChatParameters(
  ...
  const contextParams = {
    ...
    subnets: app.node.tryGetContext("subnets"),
    lb_subnets: app.node.tryGetContext("lb_subnets"),
    executeApiEndpointId: app.node.tryGetContext("execute_api_vpc_endpoint_id"),
    s3VpcEndpoint: app.node.tryGetContext("s3_vpc_endpoint_id"),
    s3EndpointIps: app.node.tryGetContext("s3_endpoint_ips"),
    certificateArn: app.node.tryGetContext("certificateArn"),
    albIngressCidr: app.node.tryGetContext("alb_ingress_cidr"),
    openSearchVpcEndpoint: app.node.tryGetContext("open_search_vpc_endpoint_id")
  };
...
export function resolveApiPublishParameters(): ApiPublishParameters {
  const envVars = {
    ...
    vpcId: getEnvVar("VPC_ID"),
    subnets: getEnvVar("SUBNETS"),
    executeApiEndpointId: getEnvVar("EXECUTE_API_ENDPOINT")
  };
